import flask as fl
from update import xlsx_downloader, xlsx_to_db, remove_imgs, download_pdns, pdn_to_pngs
        ...
        for chapter, maps in chapters.items():
            for map_meta, features_meta in maps.items():
                features[f"{tome}_{chapter[0]}{'.' if map_meta[0] else ''}{map_meta[0] if map_meta[0] else ''}"] = \
                    {meta[-4]: (meta[:-4] + meta[-3:]) for meta in features_meta}
    conn.close()
...
if __name__ == '__main__':
    app.run(debug=True, use_reloader=False, port=1159)
